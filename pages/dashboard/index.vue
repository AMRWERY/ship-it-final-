      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
        ...
        <div class="p-4 bg-white rounded-lg shadow-md dark:bg-black">
          ...
          <div class="flex items-center justify-between mt-4">
            <div class="flex items-center justify-center p-1 bg-red-500 rounded-full hover:bg-red-600">
              <icon name="material-symbols:shopping-cart-checkout" class="text-gray-700 h-7 w-7 dark:text-gray-200" />
            </div>
            <div class="flex items-center justify-center px-2 bg-red-500 rounded-full hover:bg-red-600">
              <p class="text-xl font-semibold text-gray-700 dark:text-gray-200">{{ totalCheckouts }}</p>
            </div>
          </div>
          <div class="mt-6 text-center">
            <nuxt-link to="/dashboard/orders" type="button" class="px-4 py-2 text-white btn-style">{{
              $t('btn.view_details')
              }}</nuxt-link>
          </div>
        </div>
        ...
        <div class="p-4 bg-white rounded-lg shadow-md dark:bg-black">
          ...
          <div class="flex items-center justify-between mt-4">
            ...
            <div class="flex items-center justify-center px-1.5 bg-red-500 rounded-full hover:bg-red-600">
              <p class="text-xl font-semibold text-gray-700 dark:text-gray-200">{{ inStockCount }}</p>
            </div>
          </div>
          <div class="mt-6 text-center">
            <nuxt-link to="/dashboard/items-stock" type="button" class="px-4 py-2 text-white btn-style">{{
        ...
        <div class="p-4 bg-white rounded-lg shadow-md dark:bg-black">
          ...
          <div class="mt-6 text-center">
            <nuxt-link to="/dashboard/users" type="button" class="px-4 py-2 text-white btn-style">{{
              $t('btn.view_details') }}</nuxt-link>
          </div>
        </div>
      </div>
              ...
              <li v-for="product in topProducts" :key="product.id" class="flex justify-between py-5 gap-x-6">
                <div class="flex min-w-0 gap-x-4">
                  ...
                  <div class="flex-auto min-w-0">
                    <p class="font-semibold text-gray-900 truncate text-sm/6 dark:text-gray-200">{{ product.title }}</p>
                    <div class="flex items-center justify-between">
                      <p class="mt-1 text-gray-500 dark:text-gray-100 text-xs/5">{{ product.brand }}</p>

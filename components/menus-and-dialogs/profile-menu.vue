        ...
        <div class="flex items-center space-s-1">
            ...
            <div class="relative" v-if="isAuthenticated">
                ...
                <div v-if="showDropdown"
                    class="absolute z-50 w-56 py-1 mt-2 bg-white rounded-md shadow-lg end-0 ring-1 ring-black ring-opacity-5 focus:outline-none"
                    role="menu">
                    <nuxt-link to="/profile" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 space-s-4"
                        role="menuitem" @click="showDropdown = false">
                        <icon name="iconoir:user" class="w-5 h-5 ms-2" />
                        <span>{{ $t('menu.profile') }}</span>
                    </nuxt-link>

                    <nuxt-link to="/messages" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 space-s-4"
                        role="menuitem" @click="showDropdown = false">
                        <icon name="tabler:messages" class="w-5 h-5 ms-2" />
                        <span>{{ $t('menu.messages') }}</span>
                    </nuxt-link>

                    <nuxt-link to="/addresses" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 space-s-4"
                        role="menuitem" @click="showDropdown = false">
                        <icon name="iconoir:map-pin" class="w-5 h-5 ms-2" />
                        <span>{{ $t('menu.addresses') }}</span>
                    </nuxt-link>

                    <nuxt-link to="/order-tracking" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 space-s-4"
                        role="menuitem" @click="showDropdown = false">
                    ...
                    <nuxt-link to="/" type="button" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 space-s-4"
                        @click="logout">

<template>
  <div>
    ...
    <Transition name="slide-fade">
      <div v-if="isOpen"
        class="fixed inset-0 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
              ...
              <div class="w-full max-w-xl mx-auto mt-10">
                ...
                <div>
                  <button type="submit" :disabled="loading" @click="handleLogin"
                    class="block w-full px-5 py-2 btn-style">
                ...
                <div v-if="loginFailed && !loginSuccessfully" class="mt-2">
                  <p class="font-normal text-center text-red-800">{{ $t('toast.failed_to_login') }}</p>
                </div>
              </div>
            </div>
          </div>

<template>
  <div>
    <div class="lg:col-span-3">
      <p class="py-4 text-xl font-semibold text-gray-900 capitalize hover:text-gray-800"
          ...
          <nuxt-link v-for="product in recommendedProducts" :key="product.id" :to="`/products/${product.id}`"
            ...
            <div
              ...
              <div class="text-center">
                <h3 class="text-sm font-bold text-white lg:text-base lg:text-gray-800">{{ $i18n.locale === 'ar' ?
                  product.titleAr :
                  product.title }}</h3>
                <h4 class="mt-2 text-sm font-bold text-blue-600 lg:text-base">{{ $n(parseFloat(product.discountedPrice)
